...
const PlantCard = ({ plant }) => {
  ...
  const handleCardClick = () => {
    navigate(`/plants/${plant.id}`, { state: { plant } });
  };

  return (
    <div
      onClick={handleCardClick}
      className="group relative bg-white rounded-xl overflow-hidden cursor-pointer shadow-sm hover:shadow-2xl transition-all duration-500 ease-out hover:-translate-y-2"
    >
      <div className="relative h-[350px] overflow-hidden bg-[#F4F5F0]">
        <img
          src={plant.image_url || "https://images.unsplash.com/photo-1501004318641-b39e6451bec6?q=80&w=3000&auto=format&fit=crop"}
          alt={plant.common_name}
          className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-110"
        />

        {/* Dark Gradient Overlay (Only appears on hover for text readability) */}
      ...
      <div className="p-6 relative">
        ...
        <div className="absolute top-6 right-6 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 delay-100">
          <div className="bg-[#C77D63] p-2 rounded-full text-white">
            <ArrowUpRight onClick={(e) => { e.stopPropagation(); navigate(`/plants/${plant.id}`, { state: { plant } }); }} size={20} />
          </div>
        </div>
