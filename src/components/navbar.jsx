...
const Navbar = () => {
  ...
  const showLogo = !isAuthPage && !isPlantDetailPage;
  useEffect(() => {
    setIsOpen(false);
  ...
  return (
    <>
      <div
        className={`fixed left-0 right-0 z-[100] flex w-full transition-all duration-300 pointer-events-none ${
          isOpen
            ? "top-4 justify-center px-4"
            : isPlantDetailPage
      ...
      >
        <nav
          className={`flex items-center transition-all duration-300 pointer-events-auto
    ${
      isOpen
        ? "justify-between w-full max-w-5xl bg-white border border-gray-100 rounded-3xl shadow-lg px-6 py-3"
        : isAuthPage
          ? `w-full max-w-5xl bg-transparent border-none shadow-none px-6 py-3 ${
              showLogo ? "justify-between" : "justify-end"
            }`
          : isPlantDetailPage
            ? "w-auto bg-transparent ml-auto px-0 py-0"
  ...
  `}
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `text-xs font-sans tracking-[0.2em] transition-colors duration-300 ${
                        isActive
                          ? "text-[#1A2F1C] font-bold"
                          : "text-gray-500 hover:text-[#4A6741]"
                      }`
                    }
                  >
                    {link.name}
            ...
            {!isAuthPage &&
              ...
              (userName ? (
                <div className="hidden md:flex items-center gap-4">
                  <span className="text-[#1A2F1C] font-serif italic text-sm">
                    Hello, {userName}
                  </span>
                  <button
      ...
      <div
        className={`fixed inset-0 z-[90] bg-[#F4F5F0] transition-transform duration-500 ease-in-out ${
          ...
          {!isAuthPage &&
            (userName ? (
              <div className="flex flex-col items-center gap-4 mt-8">
                <span className="font-serif text-xl text-[#1A2F1C]">
                  Hello, {userName}
                </span>
                <button
                  onClick={() => {
